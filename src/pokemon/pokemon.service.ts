...
export class PokemonService {
  ...
  async remove(id: string) {
    ...
    if(result.deletedCount === 0) throw new BadRequestException(`Pokemo with id "${ id }" not found`)
    return result
  }

  private handleExceptions( error: any ){
    if(error.code === 11000) throw new BadRequestException(`Pokemon exists in de ${JSON.stringify(error.keyValue)}`)
    console.log(error)
